...
void i2c_multi_init(PIO pio, uint pin) {
    ...
    uint pio_irq0 = (pio == pio0 ? PIO0_IRQ_0 : pio == pio1 ? PIO1_IRQ_0 : PIO2_IRQ_0);
    uint pio_irq1 = (pio == pio0 ? PIO0_IRQ_1 : pio == pio1 ? PIO1_IRQ_1 : PIO2_IRQ_1);
    i2c_multi->length = -1;
    ...
    pio_sm_put(pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[1]) << 16) | do_ack_program_instructions[0]);
    pio_sm_put(pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[3]) << 16) | do_ack_program_instructions[2]);
...
void i2c_multi_restart(void) {
    ...
    pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[1]) << 16) | do_ack_program_instructions[0]);
    pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[3]) << 16) | do_ack_program_instructions[2]);
...
static inline void byte_handler_pio(void) {
    ...
    if(i2c_multi->status == I2C_IDLE) {
        if(!i2c_multi_is_address_enabled(received >> 1)) {
            ...
            pio_sm_put(
                i2c_multi->pio,
                i2c_multi->sm_read,
                (((uint32_t)do_ack_program_instructions[10] + i2c_multi->offset_read) << 16) | do_ack_program_instructions[9]);
            pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[1]) << 16) | do_ack_program_instructions[0]);
            pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[3]) << 16) | do_ack_program_instructions[2]);
    ...
    if(i2c_multi->status == I2C_READ) {
        pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[5]) << 16) | do_ack_program_instructions[4]);
        pio_sm_put(
            i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[7] + i2c_multi->offset_read) << 16) | do_ack_program_instructions[6]);
        pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[1]) << 16) | do_ack_program_instructions[0]);
        pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[3]) << 16) | do_ack_program_instructions[2]);
        if(receive_handler) {
            if(is_address) {
    ...
    if(i2c_multi->status == I2C_WRITE && is_address) {
        ...
        pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[5]) << 16) | do_ack_program_instructions[4]);
        pio_sm_put(
            i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[8] + i2c_multi->offset_read) << 16) | do_ack_program_instructions[6]);
        pio_sm_put(i2c_multi->pio, i2c_multi->sm_read, (((uint32_t)do_ack_program_instructions[1]) << 16) | do_ack_program_instructions[0]);
    ...
    if(i2c_multi->status == I2C_WRITE && !is_address) {
        if(i2c_multi->length == -1 || (i2c_multi->length > 0 && i2c_multi->bytes_count < i2c_multi->length + 1)) {
            ...
            pio_sm_put(i2c_multi->pio, i2c_multi->sm_write, (((uint32_t)wait_ack_program_instructions[5]) << 16) | wait_ack_program_instructions[4]);
            pio_sm_put(
                i2c_multi->pio,
                i2c_multi->sm_write,
...
static inline uint8_t transpond_byte(uint8_t byte) {
    // permutation of the order of bits in the reverse sequence
    uint8_t result;
